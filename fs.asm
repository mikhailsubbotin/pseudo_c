...
; ѕримечание к Windows 8.1
; ------------------------
; —оздава€ файл в корне системного диска, учтите, что если у ¬ашей программы не достаточно прав,
; Windows перенаправит файлы в каталог "C:\Users\<ѕользователь>\AppData\Local\VirtualStore\"!
...
_dwFlagsAndAttributes = 20
        ...
        xor     si, si ; обнул€ем регистр переменной кол-ва успешно созданных поддиректорий.
        xor     ah, ah
        jmp     .recursively_directory_creation_loop_without_increment_address
...
.recursively_directory_creation_loop_next:
        test    bl, bl
        jnz     .recursively_directory_creation_loop_without_increment_address
        xor     ebx, ebx ; в режиме создани€ директории процедура должна вернуть NULL!
...
.recursively_directory_removing_loop:
        ...
        jnz     @f
        invoke  GetLastError
        ;cmp     eax, ERROR_PATH_NOT_FOUND ; нужно проверить на разных верси€х Windows, возвращает ли система в данной ситуации ERROR_PATH_NOT_FOUND (8.1 - не возвращает).
        ;jz      @f
